...
def test_load_config_reads_all_supported_values() -> None:
    ...
    assert config.agent.system_prompt == "Custom prompt"
...
def test_repeated_loads_create_independent_configs() -> None:
    first = load_config(
        {
            "AGENT_SYSTEM_PROMPT": "First prompt",
            "LLM_MODEL": "first/model",
        }
    )
    second = load_config(
        {
            "AGENT_SYSTEM_PROMPT": "Second prompt",
    ...
    assert first.agent.system_prompt == "First prompt"
    assert first.llm.model == "first/model"
    assert second.agent.system_prompt == "Second prompt"
